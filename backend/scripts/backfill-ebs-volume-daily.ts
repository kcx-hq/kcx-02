import { syncEbsVolumeDaily } from "../src/features/ec2/scheduled-jobs/handlers/ebs-volume-daily.service.js";
...
const printUsage = (): void => {
  console.info(`
Usage:
  node dist/scripts/backfill-ebs-volume-daily.js [options]
...
Examples:
  node dist/scripts/backfill-ebs-volume-daily.js --last-days=30
  node dist/scripts/backfill-ebs-volume-daily.js --current-open-month
  node dist/scripts/backfill-ebs-volume-daily.js --tenant-id=<uuid> --start-date=2026-04-01 --end-date=2026-04-21
`);
};
...
async function main(): Promise<void> {
  ...
  console.info("Starting fact_ebs_volume_daily backfill", {
    tenantId: options.tenantId,
    startDate,
    endDate,
  });

  const result = await syncEbsVolumeDaily({
    tenantId: options.tenantId ?? undefined,
    startDate,
    endDate,
  });

  console.info("fact_ebs_volume_daily backfill completed", {
    tenantId: options.tenantId,
    startDate,
    endDate,
...
main()
  .catch((error) => {
    console.error(
      "fact_ebs_volume_daily backfill failed:",
