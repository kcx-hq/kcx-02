...
import { syncEc2CostHistoryForIngestionRun } from "../src/features/billing/services/ec2-cost-history.service.js";
...
const printUsage = (): void => {
  console.info(`
Usage:
  node dist/scripts/backfill-ec2-cost-history.js [options]
...
Options:
  ...
  --correction-mode               Rebuild frozen/adjusted months too
`);
};
...
async function main(): Promise<void> {
  ...
  console.info("Starting EC2 cost history backfill...", {
  ...
  for (const run of runs) {
    ...
    const result = await syncEc2CostHistoryForIngestionRun({
    ...
    processed += 1;
    rebuiltMonthCount += result.rebuiltMonths.length;

    console.info("Backfilled ingestion run", {
      runId,
      tenantId,
      providerId,
      billingSourceId,
      affectedMonths: result.affectedMonths,
      rebuiltMonths: result.rebuiltMonths,
      skippedMonths: result.skippedMonths,
    });
  }

  console.info("EC2 cost history backfill completed", {
...
main()
  .catch((error) => {
    console.error(
      "EC2 cost history backfill failed:",
