...
const printUsage = (): void => {
  console.info(`
Usage:
  node dist/scripts/backfill-ec2-instance-cost-daily.js [options]
...
Examples:
  node dist/scripts/backfill-ec2-instance-cost-daily.js --last-days=30
  node dist/scripts/backfill-ec2-instance-cost-daily.js --current-open-month
  node dist/scripts/backfill-ec2-instance-cost-daily.js --tenant-id=<uuid> --start-date=2026-04-01 --end-date=2026-04-21
`);
};
...
async function main(): Promise<void> {
  ...
  console.info("Starting fact_ec2_instance_cost_daily backfill", {
    tenantId: options.tenantId,
    startDate,
    endDate,
  });

  const result = await syncEc2InstanceCostDaily({
    tenantId: options.tenantId ?? undefined,
    startDate,
    endDate,
  });

  console.info("fact_ec2_instance_cost_daily backfill completed", {
    tenantId: options.tenantId,
    startDate,
    endDate,
...
main()
  .catch((error) => {
    console.error(
      "fact_ec2_instance_cost_daily backfill failed:",
