...
import { fetchRdsAuroraInventory } from "../../inventory/rds-aurora/rds-aurora-inventory.adapter.js";
...
export type SyncRdsAuroraMetricsInput = {
  tenantId: string;
...
export type SyncRdsAuroraMetricsResult = {
  tenantId: string;
  cloudConnectionId: string;
  ...
  metricsResources: number;
  usageDate: string;
  persisted: PersistRdsAuroraUtilizationResult | null;
};
...
export const syncRdsAuroraMetrics = async (
  input: SyncRdsAuroraMetricsInput,
): Promise<SyncRdsAuroraMetricsResult> => {
  ...
  const persisted = await persistRdsAuroraUtilizationDaily({
    tenantId,
    cloudConnectionId,
