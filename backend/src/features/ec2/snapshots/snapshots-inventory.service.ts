...
type SnapshotSummaryRow = {
  ...
  potentialSavings: number | string | null;
};
...
export class SnapshotsInventoryService {
  async listSnapshots(input: {
    ...
    const [sourceVolumeLookup, snapshotCostLookup] = await Promise.all([
  ...
  private async loadSummary(input: {
    ...
    const row = rows[0];
    return {
      totalSnapshotCost: toNullableNumber(row?.totalSnapshotCost) ?? 0,
      totalSnapshots: toIntOrZero(row?.totalSnapshots),
      oldSnapshots: toIntOrZero(row?.oldSnapshots),
      potentialSavings: toNullableNumber(row?.potentialSavings) ?? 0,
    };
  }
}
