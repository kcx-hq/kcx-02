...
BEGIN
  ...
  END IF;
  IF to_regclass('public.cloud_connections_v2') IS NOT NULL
     AND NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_ec2_cost_history_daily_cloud_connection_id') THEN
    ALTER TABLE ec2_cost_history_daily
...
BEGIN
  ...
  END IF;
  IF to_regclass('public.cloud_connections_v2') IS NOT NULL
     AND NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_ec2_cost_history_monthly_cloud_connection_id') THEN
    ALTER TABLE ec2_cost_history_monthly
