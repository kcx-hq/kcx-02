...
const addBillingSourceFkIfPossible = async (queryInterface: any) => {
  if (!(await hasTable(queryInterface, "raw_billing_files"))) return;
  if (!(await hasTable(queryInterface, "billing_sources"))) return;
