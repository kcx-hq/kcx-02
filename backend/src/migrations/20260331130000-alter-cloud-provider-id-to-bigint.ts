...
const getColumnType = async (queryInterface: any, tableName: string, columnName: string): Promise<string | null> => {
  try {
    const table = await queryInterface.describeTable(tableName);
...
const isBigint = (type: string | null) => Boolean(type && type.includes("BIGINT"));
const isUuid = (type: string | null) => Boolean(type && type.includes("UUID"));

const dropCloudProviderForeignKeyConstraints = async (
  queryInterface: any,
  tableName: string,
  columnName: string,
) => {
...
const addCloudProviderForeignKeys = async (queryInterface: any) => {
  await queryInterface.sequelize.query(`
DO $$
BEGIN
...
const ensureCloudProviderIndexes = async (queryInterface: any) => {
  await queryInterface.sequelize.query(`
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'billing_sources' AND column_name = 'cloud_provider_id') THEN
